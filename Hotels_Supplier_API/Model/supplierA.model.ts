...
export class SupplierAModel {
    ...
    async mockGetHotelsAsyncCall(cityName:string,minPrice?:number,maxPrice?:any)
    {
        try {            
            ...
            if(params.length > 0)
                hotelsData = await this.redisClient.zRangeByScore("hotels_by_price_supplier_A:"+cityName,params);
            else
                hotelsData = await this.redisClient.zRange("hotels_by_price_supplier_A:"+cityName);
        ...
        try {            
            ...
            if(hotelsData && hotelsData.data && hotelsData.data.length > 0)
            {
                ...
                hotelsByMappedCity.forEach((hotel:any)=> {
                    ...
                    this.redisClient.zAdd(hotel?.price,`hotel:${hotel?.hotelId}`,"hotels_by_price_supplier_A:"+cityName);
                })
                if(minPrice)
   ...
   private fileRead()
    {        
        ...
        return new Promise(async(resolve,reject)=>{
            try {
                let data:any = "";                
                const fileReader = await fs.open("./Hotels_Supplier_API/Demo_JSON_Data/SupplierA.json","r");                
